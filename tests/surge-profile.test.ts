...
describe("Surge-compatible profiles", () => {
  it("builds a complete Surge profile around converted proxy lines", () => {
    const output = createSurgeProfile(nodeResource);
    ...
    expect(output).toContain(
      "RULE-SET,https://rules.flacier.com/rules/ai/openai.list,AI,update-interval=86400",
    );
  ...
  it("builds a Surfboard profile with the same routing policies", () => {
    const output = createSurfboardProfile(nodeResource);
